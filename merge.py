...
path='D:/projects/analyse-pb/merging'
try:
    os.remove(path+'/InJOIN.xlsx')
    os.remove(path+'/NotInJOIN.xlsx')
except:
    pass


Columns_table=pd.read_excel(io=path+'/readed_file.xlsm', sheet_name='Columns')
...
Measure_table=pd.read_excel(io=path+ '/readed_file.xlsm', sheet_name='DAX Expressions')
...
Combined_csv=pd.read_csv(path+'/RESULT.csv', encoding='utf-16', sep='\t')
